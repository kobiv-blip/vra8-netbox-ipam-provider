...
def allocate_in_range(range_id, auth_credentials, resource, allocation, context, endpoint):
    ...
    # Debug logging
    logging.info(f"Available IPs URL: {str(r['url'])}available-ips/?limit=5")
    logging.info(f"Available IPs Status Code: {addresses.status_code}")
    logging.info(f"Available IPs Response Text: {addresses.text}")
    logging.info(f"Available IPs Response : {r}")
    
    if addresses.status_code != 200:
        raise Exception(f"Failed to get available IPs. Status code: {addresses.status_code}, Response: {addresses.text}")
    
    try:
        addresses_json = addresses.json()
        logging.info(f"addresses json : {addresses_json}")
    except Exception as json_error:
        raise Exception(f"Failed to parse available IPs JSON response: {str(json_error)}. Response text: {addresses.text}")
    ...
    for address in addresses_json:
        ...
        if str(ip) != str(network[1]):
            ...
            # Only add VRF if it's not null
            if vrf_id is not None:
                payload["vrf"] = str(vrf_id)
                
            logging.info(f"payload for post : {payload}")
            logging.info(f"payload url: {str(netbox_url)}")
